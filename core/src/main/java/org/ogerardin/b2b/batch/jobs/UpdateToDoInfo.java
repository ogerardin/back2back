...
@JobScope
@Slf4j
public class UpdateToDoInfo extends BackupSetAwareBean implements StepExecutionListener {
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        if (exitCode.equals(ExitStatus.COMPLETED.getExitCode())) {
            int fileCount = backupJobContext.getToDoFiles().fileCount();
            long byteCount = backupJobContext.getToDoFiles().getByteCount();
